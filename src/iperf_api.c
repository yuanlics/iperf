...
void
iperf_set_test_pktdir(struct iperf_test *ipt, const char *pktdir)
{
    ipt->pktdir = strdup(pktdir);
}
...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
    ...
    {"pktdir", required_argument, NULL, OPT_PKTDIR},
...
/*
 * Open the file specified by current time and set test->pktfile to its' FD.
 */
int iperf_open_pktfile(struct iperf_stream *sp)
{
    ...
    strftime(buffer, 200, "/%Y%m%d_%H%M%S", info);

    strcpy(filename, sp->test->pktdir);
    strcat(filename, buffer);

    sprintf(addr, "_%s_%s_%d_%d_%lu.log", sp->test->ipr, sp->test->ipl, sp->test->rport, sp->test->lport, sp->settings->rate/1000000);
    strcat(filename, addr);

    sp->test->pktfile = fopen(filename, "a+");
    setbuf(sp->test->pktfile, NULL);
    if (sp->test->pktfile == NULL) {
        i_errno = IEPKTFILE;
        return -1;
    }
...
connect_msg(struct iperf_stream *sp)
{
    ...
    sp->test->ipl = strdup(ipl);
    sp->test->lport = lport;
    sp->test->ipr = strdup(ipr);
    sp->test->rport = rport;
    //printf("%c %d\n", sp->test->role, sp->test->reverse);test->role == 'c'
    if ( (sp->test->pktdir && sp->test->role=='c' && sp->test->reverse==1) || (sp->test->pktdir && sp->test->role=='s' && sp->test->reverse==0) )
        if (iperf_open_pktfile(sp) < 0)
            fprintf(stderr, "cannot open packet log file\n");
}
