...
iperf_udp_recv(struct iperf_stream *sp)
{
	...
	if (sp->test->pktdir)
	{
		unsigned long send_usec, recv_usec;
		send_usec = sent_time.secs * 1000000 + sent_time.usecs;
		recv_usec = arrival_time.secs * 1000000 + arrival_time.usecs;
		// fprintf(sp->test->pktfile, "%s %s %d %d %" PRIu64 " %d %u %u %u %u \n", sp->test->ipr, sp->test->ipl, sp->test->rport, sp->test->lport, pcount, r, sent_time.secs, sent_time.usecs, arrival_time.secs, arrival_time.usecs);
		// fprintf(sp->test->pktfile, "%" PRIu64 " %d %u %u %u %u\n", \
		// 	pcount, r, sent_time.secs, sent_time.usecs, arrival_time.secs, arrival_time.usecs);
		fprintf(sp->test->pktfile, "%" PRIu64 " %d %lu %lu\n", \
			pcount, r, send_usec, recv_usec);
	}
